...
function ensureLoaded() {
  ...
  cache.inventory.forEach((it) => {
    ...
    if (!it.area) {
      // Area is normally an explicit Excel column, but that column is
      // itself just a formula off the tag's first 3 digits (e.g.
      // "703-FE/FIT-1101" -> area "703") — see lib/areaCodes.js. Apply the
      // same rule here so items imported before this existed (or from a
      // workbook whose cached formula result was blank) still get an area.
      const derived = deriveAreaFromTag(it.tag);
      if (derived) {
        it.area = derived;
        backfilled = true;
      }
...
function addInventoryItem(fields) {
  ...
  const item = {
    ...
    area: fields.area || deriveAreaFromTag(tag) || '',
