...
const { deriveAreaFromTag } = require('./areaCodes');
...
function parseReceivedItems(buffer) {
  ...
  dataRows.forEach((row, i) => {
    ...
    // The AREA column is itself an Excel formula off the tag
    // (=IF(AND(LEN(tag)>=4,MID(tag,4,1)="-",ISNUMBER(VALUE(LEFT(tag,3)))),LEFT(tag,3),"")).
    // A workbook uploaded without being recalculated first can have a
    // stale/blank cached formula result in that cell, so whenever the
    // explicit Area value comes through blank, derive it ourselves from
    // the tag rather than silently importing a blank area.
    const explicitArea = String(get('area') ?? '').trim();
    const area = explicitArea || deriveAreaFromTag(tag) || '';
    rows.push({
